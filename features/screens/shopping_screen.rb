class ShoppingScreen
  ...
  def initialize
    ...
    @check_cart_button = Elements.new(:id, 'product_header_item_current_price')
    ...
    @added_cart_item_price = Elements.new(:xpath, :"//android.widget.TextView[@resource-id='pl.com.fourf.ecommerce:id/cart_product_item_title']")
  ...
  def verify_added_items
    ...
    raise "ELEMENT WAS NOT ADDED: #{@added_item_name}" unless @added_cart_item_name == @added_item_name
  end

  def verify_added_item_price
    unless @added_cart_item_price == @shopping_cart_product_value
      raise "ELEMENT PRICE NOT THE SAME: #{@added_cart_item_price}"
    end
  end
end
